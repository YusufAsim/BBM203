...
std::vector<std::string> &RealmShaper::getVisitedIsles()
{
    return this->visitedIsles;
}
...
std::vector<RealmShaper *> RealmShaper::readFromFile(const std::string &filename)
{
    ...
    if (!file.is_open())
    {
        std::cerr << filename << "can not be oppened.";
    }
    else
    {
        std::string line;
        ...
        {
            std::stringstream player_info(line);
            std::string name;
            int honour_point;
            player_info >> name >> honour_point;
            players.push_back(new RealmShaper(name, honour_point));
        }
    }
    file.close();
    return players;
